...
- **Next Milestones**: packaging, installer/autostart polish, settings UI for shortcuts, Worker URL, provider, model, and optional voice re-enable
...
| `worker/src/index.ts` | ~919 | Cloudflare Worker proxy. Four routes: `/chat` (Anthropic/OpenAI/Grok/Gemini vision chat with normalized streaming), `/computer-use` (non-streaming Gemini Computer Use that returns structured FunctionCalls + final text), `/tts` (ElevenLabs), `/transcribe-token` (AssemblyAI temp token). |
...
| `Buddy.Windows/TextInput/TextPromptWindow.xaml` | ~84 | Focusable WPF text prompt shown near the cursor, with typed input, Send/Cancel controls, Enter-to-send, Shift+Enter newline, and Escape-to-close behavior. |
| `Buddy.Windows/TextInput/TextPromptWindow.xaml.cs` | ~146 | Positions the typed prompt near the current cursor, handles keyboard/button submission, switches between Ask Buddy / Act on my desktop modes, and publishes submitted prompt text. |
...
| `Buddy.Windows/Tray/TrayIconManager.cs` | ~304 | Windows tray icon lifecycle. Handles left-click panel toggling, right-click context menu, panel positioning, shortcut/capture/transcription/screen-capture/AI/TTS-state tray text, and cleanup. |
| `Buddy.Windows/Tray/FloatingPanelWindow.xaml` | ~208 | Borderless, topmost WPF floating panel used by the Windows tray shell. Shows push-to-talk/typed-input shortcuts, microphone capture, live audio level, transcript text, and Claude response text. |
| `Buddy.Windows/Tray/FloatingPanelWindow.xaml.cs` | ~589 | Applies no-activate/tool-window Win32 styles to the Windows floating panel, reflects push-to-talk/microphone/transcription/screen-capture/AI/TTS state changes, and handles the quit button. |
| `Buddy.Windows/Voice/PushToTalkHotkeyMonitor.cs` | ~415 | Windows low-level keyboard hook for `Ctrl + Alt + Space` typed prompt detection, `Ctrl + Alt + Esc` shutdown, `Ctrl + Alt + A` Computer Use action-mode trigger, and optional `Ctrl + Alt` push-to-talk press/release detection when voice mode is enabled. |
